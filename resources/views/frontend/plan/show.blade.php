        ...
        <div class="green-bg">
            <div class="container p-3 p-md-5">
                <div class="text-start text-white ">
                    <h3 class=" border-top border-dark border-5 col-8 col-md-4 col-lg-2  fw-bold pt-2 ">{{$product->title}}</h3>
                    <h6><a href="{{url('service/plan')}}" class="link-light">Service Plan</a>/ <a href="" class="link-light">Single</a></h6>
                </div>
            </div>
        </div>
                    ...
                    <div class="mb-5" id="finance">
                        ...
                        @if(!empty($planpdf))
                            <ul class="list-group list-group-flush">
                                @foreach($planpdf as $plan)
                                    <li class="list-group-item bg-transparent">
                                        <a href="" class="link-dark">{{$plan->title}}</a>
                                    </li>
                                @endforeach
                            </ul>
                        @endif
                    ...
                    <div class="ps-0 ps-md-0 ps-lg-5 ms-0 ms-md-5">
                        ...
                        <div class="">
                            <form action="{{url('send/plan/inquiry')}}" method="post">
                                {{csrf_field()}}
                                <input type="hidden" name="plan_name" value="{{$product->title}}">
                                <div class="row">
